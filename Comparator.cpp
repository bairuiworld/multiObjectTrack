...
void Comparator::save() const {
    ...
    if (f.is_open()) {
        ...
        f << "Re: " << double(tp)/(tp+fn)<<"\n";
        f << "Sp: " << double(tn)/(tn+fp)<<"\n";
        f << "FPR:" << double(fp)/(fp+tn)<<"\n";
        f << "FNR:" << double(fn)/(tn+fp)<<"\n";
        f << "PWC:" << double(fn+fp)/(tp+fn+fp+tn)*100<<"\n";
        f << "Pr :" << double(tp)/(tp+fp)<<"\n";
        f.close();
    } else {
